...
public:
    MapRunner(FT::Feldespato & fdp)
        ...
        m_climbing(0), m_climbed_tile(false), m_climb_end_perc(0.5),
    ...
    void Update(FT::Feldespato & fdp, Player & player)
    {
        ...
        if (m_climbing)
        {
            m_climb_offset += glm::sign(m_climbing)   * (m_mov_speed / m_climb_end_perc);
            m_pos.z = m_pos.z + glm::sign(m_climbing) * (m_mov_speed / m_climb_end_perc);
